...
//OTP Sent Data
$sql1_exe = $main_app->sql_run("SELECT OTP_REQ_ID, OTP_MOBILE_NUM FROM LOG_OTPREQ WHERE OTP_REQ_ID = :OTP_REQ_ID", array( 'OTP_REQ_ID' => $_SESSION['OTP_REQ_ID'] ));
...
$MobileNumber = (isset($otp_data['OTP_MOBILE_NUM']) && strlen($otp_data['OTP_MOBILE_NUM']) == "10") ? $main_app->mask_mobile($otp_data['OTP_MOBILE_NUM']) : "XXXXXXXXXX";
  ...
  <div class="row">
    ...
    <input type="hidden" id="otp_key" value="<?php echo $safe->rsa_public_key();?>" />
      ...
      <div class="p-2">
        ...
        <div class="h5 text-center text-primary mt-3 mb-2">Verify your Mobile number</div>
        <div class="text-center text-secondary mb-4 px-4">Please enter the 6 digit OTP verification code that has been sent to your mobile number <?php echo $MobileNumber; ?> via SMS.</div>
        ...
        <div class="row px-1 mb-4">
          <div class="col-md-6 form-group text-center text-md-left">
            <button type="submit" class="btn h-btn3 m-0 px-4 py-2" id="sbt" name="sbt" tabindex="7">Verify SMS OTP <span class="mdi mdi-arrow-right" aria-hidden="true"></span></button>
          </div>
          <div class="col-md-6 form-group text-center text-md-right">
            <button class="btn btn-link btn-sm text-dark disabled small" onclick="resendSmsOtp(); return false;" name="ResendBtn" id="ResendBtn" tabindex="-1">Resend OTP</button>
          </div>
        </div>
...
?>
  ...
  <script type="text/javascript">
    ...
    function resendSmsOtp() {
      on_change('resend_sms_otp','modify');
    }
