...
// IF Invalid Request
if( !isset($_GET['data']) || !isset($_SESSION['APP_TOKEN']) || $main_app->strsafe_input($_GET['data']) !== $_SESSION['APP_TOKEN'] ) {
	exit('');
}
...
class TextClass {
	...
	public function phptext($text,$textColor,$backgroundColor='',$fontSize,$imgWidth,$imgHeight,$dir,$fileName) {
		...
		/* settings */
		//$font = DIRPATH . '/fonts/calibri.ttf'; // define font
	...
	public function phpcaptcha($textColor,$backgroundColor,$imgWidth,$imgHeight,$noiceLines=0,$noiceDots=0,$noiceColor='#162453') {	
		/* Settings */
		$text=$this->random();
		//$font = DIRPATH . '\fonts\monofont.ttf'; // font
...
$textObj->phpcaptcha('#7C0A02','#fff',120,40,5,30);
ob_end_flush();
