    ...
    public function sql_update_data($tableName,$data,$condition) {
        try {
            if($tableName != NULL && is_array($data) && is_array($condition)) {
                ...
                foreach($all_params as $key3 => $value3) { $pdoStatement->bindValue(':'.$key3, $value3, PDO::PARAM_STR); }
                $result = $pdoStatement->execute(); // Execute SQL
                return $result;
            } else {
            ...
            //die($e->getMessage());
            error_log(basename($_SERVER['PHP_SELF']));
            error_log("TAB: ".$tableName." | SQL: ".$sql." | DATA: ".print_r($data, true)." | CON: ".print_r($condition, true));
            error_log($e->getMessage());
            return false;
        }
    }
